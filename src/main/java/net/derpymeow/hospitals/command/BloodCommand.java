...
import net.derpymeow.hospitals.procedures.TogglebloodlosspersonProcedure;
	...
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("blood")
				...
				.then(Commands.literal("type").then(Commands.literal("get").then(Commands.argument("name", EntityArgument.player()).executes(arguments -> {
					...
					return 0;
				})).then(Commands.literal("toggle_blood_loss").then(Commands.argument("name", EntityArgument.player()).then(Commands.argument("bltf", BoolArgumentType.bool()).executes(arguments -> {
					Level world = arguments.getSource().getUnsidedLevel();
					double x = arguments.getSource().getPosition().x();
					double y = arguments.getSource().getPosition().y();
					double z = arguments.getSource().getPosition().z();
